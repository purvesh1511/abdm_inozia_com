...
.theme-modal-header{background-color: #bd0745;padding: 10px 15px; color: #fff; border-radius: 5px 5px 0px 0px;}
.close-white{    
    opacity: 100;
    text-shadow: none;
...
<script>
     function getSlotByShift(shift){
        ...
        $.ajax({
                url: '<?php echo base_url(); ?>welcome/chackvalidation',
                type: "POST",
            ...
            success: function(res){
                if(res.length){
                    ...
                    $.each(res, function (i, obj)
                    {
                        ...
                        div.onclick = function() {getSlotByShift(obj.id);  };
