   ...
   <div class="col-md-10">
    <div class="box box-primary" id="tachelist">
     <div class="box-header ptbnull">
      <h3 class="box-title titlefix"><?php echo $this->lang->line('company_list') ;?></h3>
      <div class="box-tools pull-right">
	  <?php if ($this->rbac->hasPrivilege('medicine_company', 'can_add')) { ?>
       <a onclick="addModal()" class="btn btn-primary btn-sm medicine"><i class="fa fa-plus"></i> <?php echo $this->lang->line('add_company') ;?></a>
	  <?php } ?>
      </div>
     </div>
     <div class="box-body">
      ...
      <div class="download_label"><?php echo $this->lang->line('company_list') ;?></div>
       <table class="table table-striped table-bordered table-hover example">
        ...
        <tbody>
         ...
         <tr>
          ...
          <td class="text-right">
			<?php if ($this->rbac->hasPrivilege('medicine_company', 'can_edit')) { ?>
           <a onclick="editmyModal(<?php echo $value['id'] ?>,'<?php echo $value['company_name']; ?>')" class="btn btn-default btn-xs" data-toggle="tooltip" title="<?php echo $this->lang->line('edit'); ?>">
            <i class="fa fa-pencil"></i>
           </a>
			<?php } ?>
		   
		   <?php if ($this->rbac->hasPrivilege('medicine_company', 'can_delete')) { ?>
    ...
    <div class="modal-dialog modal-mid" role="document">
        <div class="modal-content modal-media-content mx-2">
            <div class="modal-header modal-media-header overflow-hidden">
                ...
                <h4 class="modal-title"><?php echo $this->lang->line('add_company') ;?></h4> 
            </div>            
                <form id="form_add_multiple" action="<?php echo site_url('admin/medicineunit/add_multiple_company') ?>" class="ptt10" method="post" accept-charset="utf-8">
                                    ...
                                    <table class="table table-striped mb0 table-bordered table-hover tablefull12 tblProducts" id="tableID_vitals">
                                        <thead>
                                            <tr class="font13 white-space-nowrap">
                                                <th width="90%"><?php echo $this->lang->line('company_name'); ?><small class="req" style="color:red;"> *</small></th>
                                                <th width="10%"></th>
                                            </tr>
                                        </thead>
                                        <tbody  id="set_row">                                       
 ...
 <div class="modal-dialog modal-mid" role="document">
  <div class="modal-content modal-media-content mx-2">
   <div class="modal-header modal-media-header">
    <button type="button" class="close" data-dismiss="modal">&times;</button>
    <h4 class="modal-title" id="company_title"><?php echo $this->lang->line('add_company'); ?></h4>
   </div>
   <form id="formadd" action="<?php echo site_url('admin/medicineunit/add_company') ?>" method="post" accept-charset="utf-8">
       ...
       <div class="row">
        <div class="col-sm-12">
         <div class="form-group">
          ...
          <input autofocus="" name="company_name" id="company_name" placeholder="" type="text" class="form-control" />
...
<script>
    ...
    function editmyModal(id,company_name){
        $("#myModal").modal('show');
        $("#company_title").text("<?php echo $this->lang->line('edit_company'); ?>");
        $("#company_name").val(String(company_name));
        $("#id").val(id);
    }

    function deletepharmacyunit(id){
        delete_recordByIdReload('admin/medicineunit/deletecompany/'+id, '<?php echo $this->lang->line('delete_confirm'); ?>');
    }
</script>
